...
## 6.5. DB 스키마 마이그레이션 (v1.7 추가)

> ⚠️ **이 마이그레이션은 기존 4번 스키마가 실행된 후 적용합니다.** 이미 생성된 테이블에 새 컬럼을 추가하는 ALTER TABLE 문입니다.

Supabase Dashboard → **SQL Editor** → 새 쿼리:

```sql
-- ============================================
-- v1.7 Migration: 영어 설명 + 사용자 표시 설정
-- ============================================

-- 1. words 테이블: 영어 설명 컬럼 추가 (영영 사전 모드)
ALTER TABLE public.words
  ADD COLUMN IF NOT EXISTS description_en TEXT;

-- 2. users 테이블: 단어 카드 표시 설정 (필드 선택 + 순서)
ALTER TABLE public.users
  ADD COLUMN IF NOT EXISTS display_preferences JSONB DEFAULT '{
    "visibleFields": ["description", "description_en", "image_text", "teps_point", "synonyms", "antonyms", "paraphrasing", "comparisons", "example"],
    "fieldOrder": ["description", "description_en", "image_text", "teps_point", "synonyms", "antonyms", "paraphrasing", "comparisons", "example"],
    "searchMode": 1
  }'::jsonb;
```

> **display_preferences 필드 설명:**
> - `visibleFields`: 사용자가 단어 카드에서 보고 싶은 필드 목록 (체크박스 선택)
> - `fieldOrder`: 필드 표시 순서 (드래그로 변경 가능)
> - `searchMode`: 동시 검색 단어 수 (1~4, 기본값 1)
