...
## Phase 39: 오답노트 상세 리뷰 (완료)
**완료 일시:** 2026-02-28
**커밋:** `b4d554a`

### 구현 내용 (`/quiz/page.tsx`) ✅
- 오답 카드마다 ChevronDown 펼치기/접기 토글 버튼 추가
- 펼치면 4개 선택지 전체 표시 (정답: 초록, 내 오답: 빨강, 나머지: 흐림)
- 헤더에 "전체 펼치기/접기" 버튼 추가 (모든 오답 일괄 제어)
- 시간초과(`selectedIndex === -1`) 경우 정답만 초록으로 표시
- 새 퀴즈 시작 시 `expandedDetails` 초기화

**Phase 39 총 커밋:** 1개

---

## Phase 40: 퀴즈 영→한 보기 랜덤 뜻 선택 (완료)
**완료 일시:** 2026-02-28
**커밋:** `67b002d`

### 배경
영→한 단어 퀴즈에서 4개 보기의 한글 뜻이 항상 `meanings[0]`(첫 번째 뜻)만 표시되어 다양한 뜻 학습이 불가능했음.

### 구현 내용 (`/api/quiz/route.ts`) ✅
- en→ko 보기 생성 시 `meanings[0]` → `meanings[]` 배열에서 랜덤 인덱스로 선택
- 정답/오답 4개 보기 모두 동일하게 적용
- `meanings`가 비어있으면 `word` 텍스트로 fallback (기존 안전장치 유지)
- 퀴즈 시작마다 다른 뜻 조합으로 출제되어 다양한 의미 학습 가능
